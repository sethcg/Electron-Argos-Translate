...
export const LanguageSelect: FunctionComponent<Props> = ({ isSource, storeKey, title }) => {
    ...
    useEffect(() => {
        ...
        async function getPackages() {
            ...
            // REMOVE DUPLICATES
            languages = languages.filter((a: SimpleLanguage, index: number, array: SimpleLanguage[]) => 
                array.findIndex((b: SimpleLanguage) => (b.code === a.code)) === index
            )
            // SORT BY NAME
            ...
            if(selectedIndex > 0) {
                ...
                selectList.push(<option key={0} value={selected[0].code}>{selected[0].name}</option>);
            }
            for (let index = 0; index < languages.length; index++) {
                const code = languages[index].code;
                const name = languages[index].name;
                selectList.push(<option key={index + 1} value={code}>{name}</option>);
            }
